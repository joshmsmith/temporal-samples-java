...
public class TransactionWorkflowNoDelayImpl implements TransactionWorkflowNoDelay {
  ...
  public TxResult processTransaction(TransactionRequest txRequest) {
    this.tx = activities.mintTransactionIdNoDelay(txRequest);

    try {
      this.tx = activities.initTransactionNoDelay(this.tx);
    } catch (Exception e) {
      initError = e;
    } finally {
      initDone = true;
    ...
    if (initError != null) {
      ...
      activities.cancelTransactionNoDelay(tx);
      return new TxResult("", "Transaction cancelled.");
    } else {
      activities.completeTransactionNoDelay(this.tx);
