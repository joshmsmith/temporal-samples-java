...
public class TransactionWorkflowMoreDelayImpl implements TransactionWorkflowMoreDelay {
  ...
  public TxResult processTransaction(TransactionRequest txRequest) {
    this.tx = activities.mintTransactionIdMoreDelay(txRequest);

    try {
      this.tx = activities.initTransactionMoreDelay(this.tx);
    } catch (Exception e) {
      initError = e;
    } finally {
      initDone = true;
    ...
    if (initError != null) {
      ...
      activities.cancelTransactionMoreDelay(tx);
      return new TxResult("", "Transaction cancelled.");
    } else {
      activities.completeTransactionMoreDelay(this.tx);
